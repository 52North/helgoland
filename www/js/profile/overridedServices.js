angular.module('n52.core.profile')
  .config(['$provide',
    function($provide) {
      ...
      $provide.decorator('SetInternalTimeseriesService', ['$delegate', 'permalinkEvaluationService', 'seriesApiInterface', 'timeseriesService', 'profilesService', 'utils',
        function($delegate, permalinkEvaluationService, seriesApiInterface, timeseriesService, profilesService, utils) {
          $delegate.setsParameters = function() {
            ...
            if (timeseries) {
              ...
              angular.forEach(timeseries.split(","), function(internalID) {
                ...
                if (Object.keys(comb).length > 0) {
                  seriesApiInterface.getTimeseries(comb.id, comb.apiUrl).then(function(data) {
                    switch (data.type) {
                      case 'profile':
